  ...
  const filtered = useMemo(() => {
    ...
    return attributes.filter(
      (a) =>
        ...
        a.fitType?.toLowerCase().includes(query) ||
        a.sleeveType?.toLowerCase().includes(query) ||
        a.collarStyle?.toLowerCase().includes(query) ||
        a.countryOfOrigin?.toLowerCase().includes(query) ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = attributeId
        ? `${API_BASE_URL}/api/product-attributes/${attributeId}`
        : `${API_BASE_URL}/api/product-attributes`;
  ...
  return (
    <div className="p-4 bg-white rounded-xl shadow max-w-6xl mx-auto">
      <h2 className="text-xl font-bold mb-4">
        {attributeId ? "Edit Product Attribute" : "Product Attribute Manager"}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-3">
        ...
        <div>
          ...
          {errors.countryOfOrigin && (
            <p className="text-sm text-red-600">{errors.countryOfOrigin}</p>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold">
          {loading ? "Loading..." : `Attributes (${attributes.length})`}
        </h3>
        <input
