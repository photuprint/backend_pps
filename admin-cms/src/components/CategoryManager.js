...
const CategoryManager = () => {
  ...
  const handleEdit = (category) => {
    setFormData({
      ...
      image: category.image || null, // Preserve existing image
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      if (existingCategory) {
        ...
        const suggestion = existingCategory.isActive ? 
          `Consider deleting the active category "${existingCategory.name}" first, or use a different name for the restored category.` :
          `Consider deleting the inactive category "${existingCategory.name}" first, or use a different name for the restored category.`;
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.log('Categories with categoryId:', processedCategories.filter(cat => cat.categoryId));
  ...
  return (
    <div className="paddingAll20">
      ...
      <PageHeader
        ...
        subtitle="Manage your product categories and classifications"
        isEditing={!!editingId}
        ...
        <div className="listHeader makeFlex spaceBetween end appendBottom24">
          <div className="leftSection">
            ...
            <StatusFilter
              ...
              counts={calculateStandardStatusCounts(categories)}
              disabled={loading}
            />
          </div>
        ...
        {filteredCategories.length === 0 && !loading ? (
          <div className="emptyState textCenter paddingAll60">
            ...
            <div className="font14 grayText">
              <p>💡 <strong>Category ID Format:</strong> PPSCATNM1001, PPSCATNM1002, etc.</p>
                ...
                {displayedCards.map((category) => (
                  <EntityCard
                      ...
                      return (
                        <>
                          ...
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                            ...
                            <span className={`detailValue font14 ${category.deleted ? 'deleted' : (category.isActive ? 'greenText' : 'inactive')} appendLeft6`}>
                              {category.deleted ? 'Deleted' : (category.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </div>
                        </>
                      );
                    ...
                    renderActions={(category) => (
                      <ActionButtons
                        onEdit={category.deleted ? undefined : () => handleEdit(category)}
                        onDelete={() => handleDelete(category._id || category.id)}
                        onRevert={category.deleted ? () => handleRevert(category._id || category.id) : undefined}
                        loading={loading}
                        size="normal"
                        editText="✏️ Edit"
                        deleteText={category.deleted ? "🗑️ Final Del" : "🗑️ Delete"}
                        revertText="🔄 Undelete"
                        editTitle="Edit Category"
                        deleteTitle={category.deleted ? "Permanently delete this category" : "Mark category as deleted"}
                        revertTitle="Restore this category back to active"
                        editDisabled={category.deleted}
                      />
                    )}
                    ...
                    <tbody>
                      {currentCategories.map((category) => (
                        <tr key={category._id} className="tableRow">
                          ...
                          <td className="tableCell">
                            <span className={`statusText ${category.deleted ? 'deleted' : (category.isActive ? 'active' : 'inactive')}`}>
                              {category.deleted ? 'Deleted' : (category.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </td>
                          <td className="tableCell">
                          ...
                          <td className="tableCell">
                               ...
                               <ActionButtons
                                 ...
                                 editTitle="Edit Category"
                                 deleteTitle={category.deleted ? "Final Del" : "Delete Category"}
                                 revertTitle="Restore this category back to active"
                                 editDisabled={category.deleted}
                               />
                             </div>
