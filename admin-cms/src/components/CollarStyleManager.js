...
const CollarStyleManager = () => {
  ...
  const filteredCollarStyles = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(collarStyle => 
        collarStyle.name.toLowerCase().includes(query) ||
        (collarStyle.description && collarStyle.description.toLowerCase().includes(query))
      );
    }
  ...
  return (
    <div className="paddingAll20">
      ...
      <StickyHeaderWrapper>
        <PageHeader
          title="Collar Style Management"
          subtitle="Manage your product collar styles and classifications"
          isEditing={!!editingId}
          editText="Edit Collar Style"
          createText="Add New Collar Style"
        />
      </StickyHeaderWrapper>
      ...
      <div className="brandFormContainer paddingAll32 appendBottom30" ref={formRef}>
        <form onSubmit={handleSubmit} className="brandForm">
          ...
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ref={collarStyleNameInputRef}
                type="text"
                name="name"
                label="Collar Style Name"
                value={formData.name}
                onChange={handleChange}
                placeholder="Enter Collar Style Name (e.g., Mandarin Collar)"
                required={true}
              />
            </div>
          </div>
          ...
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ...
                onChange={handleChange}
                placeholder="Enter Collar Style Description"
                rows={3}
              />
            </div>
          </div>
          ...
          <div className="formActions paddingTop16">
            ...
            >
              {loading ? (
                ...
                <span>{editingId ? "Update Collar Style" : "Add Collar Style"}</span>
              )}
            </button>
        ...
        <div className="listHeader makeFlex spaceBetween alignCenter appendBottom24">
          <div className="leftSection">
            ...
            <StatusFilter
              ...
              counts={calculateStandardStatusCounts(collarStyles)}
              disabled={loading}
            />
          </div>
          <div className="rightSection makeFlex alignCenter gap16">
            ...
            <SearchField
              ...
              placeholder="Search collar styles..."
              disabled={loading}
              minWidth="250px"
                ...
                {displayedCards.map((collarStyle) => {
                  ...
                  return (
                    <EntityCard
                      ...
                      renderActions={(collarStyle) => (
                        <ActionButtons
                          onEdit={collarStyle.deleted ? undefined : () => handleEdit(collarStyle)}
                          onDelete={() => handleDelete(collarStyle._id)}
                          onRevert={collarStyle.deleted ? () => handleRevert(collarStyle._id) : undefined}
                          loading={loading}
                          size="normal"
                          deleteText={collarStyle.deleted ? "🗑️ Final Del" : "🗑️ Delete"}
                          deleteTitle={collarStyle.deleted ? "Permanently delete this collar style" : "Mark collar style as deleted"}
                          revertText="🔄 Restore"
                          revertTitle="Restore this deleted collar style"
                        />
                      )}
                      className="brandCard"
                    />
                          ...
                          <tr key={collarStyle._id} className="tableRow">
                            ...
                            <td className="tableCell">
                              <span className="dateText">
                                {new Date(collarStyle.createdAt).toLocaleDateString()}
                              </span>
                            </td>
                            <td className="tableCell">
                              <div className="tableActions makeFlex gap8">
                                <ActionButtons
                                  ...
                                  size="small"
                                  editText="✏️"
                                  deleteText={collarStyle.deleted ? "🗑️" : "🗑️"}
                                  revertText="🔄"
                                  editTitle="Edit Collar Style"
                                  deleteTitle={collarStyle.deleted ? "Final Del" : "Delete Collar Style"}
                                  revertTitle="Restore Collar Style"
                                />
                              </div>
                            </td>
                          </tr>
