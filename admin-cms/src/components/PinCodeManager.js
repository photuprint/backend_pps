...
const PinCodeManager = () => {
  ...
  const validatePinCodeName = (name) => {
    ...
    const existingPinCode = pinCodes.find(pinCode => 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        // Update pin code
        await api.put(`/api/pin-codes/${editingId}`, pinCodeData);
        setSuccess(`✅ Pin Code "${formData.name.trim()}" has been updated successfully!`);
      } else {
        // Create pin code
        await api.post('/api/pin-codes', pinCodeData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (isPermanentDelete) {
        // Permanent deletion
        await api.delete(`/api/pin-codes/${pinCodeId}/hard`);
        ...
        await api.delete(`/api/pin-codes/${pinCodeId}`);
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      if (existingPinCode) {
        ...
        const suggestion = existingPinCode.isActive ? 
          `Consider deleting the active pin code "${existingPinCode.name}" first, or use a different name for the restored pin code.` :
          `Consider deleting the inactive pin code "${existingPinCode.name}" first, or use a different name for the restored pin code.`;
      ...
      await api.put(`/api/pin-codes/${pinCodeId}`, {
        name: pinCode.name,
        description: pinCode.description,
        isActive: true,
        deleted: false
  ...
  const fetchPinCodes = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/pin-codes?showInactive=true&includeDeleted=true');
  ...
  const filteredPinCodes = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(pinCode => 
        pinCode.name.toLowerCase().includes(query) ||
        (pinCode.description && pinCode.description.toLowerCase().includes(query))
      );
    }
  ...
  return (
    <div className="paddingAll20">
      ...
      <PageHeader
        ...
        subtitle="Manage your product pin codes and classifications"
        isEditing={!!editingId}
        editText="Edit Pin Code"
        createText="Add New Pin Code"
      />
        ...
        <form onSubmit={handleSubmit} className="brandForm">
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ref={pinCodeNameInputRef}
                type="text"
                name="name"
                label="Pin Code Name"
                value={formData.name}
                onChange={handleChange}
                placeholder="Enter Pin Code Name (e.g., 110001)"
                required={true}
              />
            </div>
          </div>
          ...
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ...
                onChange={handleChange}
                placeholder="Enter Pin Code Description"
                rows={3}
              />
            </div>
          </div>
          ...
          <div className="formActions paddingTop16">
            ...
            >
              {loading ? (
                ...
                <span>{editingId ? "Update Pin Code" : "Add Pin Code"}</span>
              )}
            </button>
        ...
        <div className="listHeader makeFlex spaceBetween end appendBottom24">
          <div className="leftSection">
            ...
            <StatusFilter
              ...
              counts={calculateStandardStatusCounts(pinCodes)}
              disabled={loading}
            />
          </div>
                      ...
                      return (
                        <>
                          ...
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                            ...
                            <span className={`detailValue font14 ${pinCode.deleted ? 'deleted' : (pinCode.isActive ? 'greenText' : 'inactive')} appendLeft6`}>
                              {pinCode.deleted ? 'Deleted' : (pinCode.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </div>
                        </>
                      );
                    }}
                    renderActions={(pinCode) => (
                      <ActionButtons
                        onEdit={pinCode.deleted ? undefined : () => handleEdit(pinCode)}
                        onDelete={() => handleDelete(pinCode._id || pinCode.id)}
                        onRevert={pinCode.deleted ? () => handleRevert(pinCode._id || pinCode.id) : undefined}
                        loading={loading}
                        size="normal"
                        editText="✏️ Edit"
                        deleteText={pinCode.deleted ? "🗑️ Final Del" : "🗑️ Delete"}
                        revertText="🔄 Undelete"
                        editTitle="Edit Pin Code"
                        deleteTitle={pinCode.deleted ? "Final Del" : "Delete Pin Code"}
                        revertTitle="Restore Pin Code"
                      />
                    )}
                    className="brandCard"
                  />
                    ...
                    <tbody>
                      {currentPinCodes.map((pinCode) => (
                        <tr key={pinCode._id || pinCode.id} className="tableRow">
                          ...
                          <td className="tableCell">
                            <span className={`statusText ${pinCode.deleted ? 'deleted' : (pinCode.isActive ? 'active' : 'inactive')}`}>
                              {pinCode.deleted ? 'Deleted' : (pinCode.isActive ? 'Active' : 'Inactive')}
                            </span>
                          </td>
                          <td className="tableCell">
                          ...
                          <td className="tableCell">
                            <div className="tableActions makeFlex gap8">
                              <ActionButtons
                                ...
                                size="small"
                                editText="✏️"
                                deleteText={pinCode.deleted ? "🗑️" : "🗑️"}
                                revertText="🔄"
                                editTitle="Edit Pin Code"
                                deleteTitle={pinCode.deleted ? "Final Del" : "Delete Pin Code"}
                                revertTitle="Restore Pin Code"
