...
const CountryOfOriginManager = () => {
  ...
  const fetchCountries = async () => {
    try {
      setLoading(true);
      const response = await api.get('/countries?includeDeleted=true');
  ...
  // Filter countries based on search query and status - memoized to prevent infinite loops
  ...
  useEffect(() => {
    if (viewMode === 'card' && filteredCountries.length > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.msg || "Failed to save country");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      // Check if there's already an active or inactive country with the same name
      const existingCountry = countries.find(c => 
        c._id !== countryId && // Exclude current country being reverted
        c.name.toLowerCase().trim() === country.name.toLowerCase().trim() && // Same name
      ...
      if (existingCountry) {
        ...
        const suggestion = existingCountry.isActive ? 
          `Consider deleting the active country "${existingCountry.name}" first, or use a different name for the restored country.` :
          `Consider deleting the inactive country "${existingCountry.name}" first, or use a different name for the restored country.`;
        
        setError(`❌ Cannot restore country "${country.name}". A ${status.toLowerCase()} country with this name already exists. ${suggestion}`);
        setLoading(false);
        setDeletePopup({
          isVisible: false,
          countryId: null,
      ...
      <div className="brandsListContainer paddingAll32">
        <div className="listHeader makeFlex spaceBetween alignCenter appendBottom24">
          <div className="leftSection">
            <h2 className="listTitle font30 fontBold blackText">Countries ({filteredCountries.length})</h2>
            <StatusFilter
            ...
            {viewMode === 'card' && (
              <div className="brandsGrid">
                {displayedCards.map((country) => {
                  const statusInfo = getStatusInfo(country);
                  return (
                    <EntityCard
                      key={country._id}
                      entity={country}
                      ...
                      onRevert={country.deleted ? () => handleRevert(country._id) : undefined}
                      loading={loading}
                      logoPlaceholderColor={generateEntityColor(country._id, country.name)}
                      renderDetails={(country) => (
                        <>
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                            ...
                            <span className="detailValue font14 darkGrayText">{country.code}</span>
                          </div>
                          {country.description && (
                            <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                              <span className="detailLabel font14 fontSemiBold grayText textUppercase">Description:</span>
                              <span className="detailValue font14 darkGrayText">{country.description}</span>
                            </div>
                          )}
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                      ...
                      renderActions={(country) => (
                        <ActionButtons
                          ...
                          onDelete={() => handleDelete(country._id)}
                          onRevert={country.deleted ? () => handleRevert(country._id) : undefined}
                          loading={loading}
                          size="normal"
                          deleteText={country.deleted ? "🗑️ Final Del" : "🗑️ Delete"}
                          deleteTitle={country.deleted ? "Permanently delete this country" : "Mark country as deleted"}
                          revertText="🔄 Restore"
                          revertTitle="Restore this deleted country"
                        />
                      )}
                      className="brandCard"
                    />
                  ...
                  <table className="brandsTable">
                    ...
                    <tbody>
                      {currentCountries.map((country) => {
                        const statusInfo = getStatusInfo(country);
                        return (
                          <tr key={country._id} className="tableRow">
