...
const HeightManager = () => {
  ...
  const validateHeightName = (name) => {
    if (!name || !name.trim()) {
      return { isValid: false, error: "Height Name is required" };
    }
    ...
    const existingHeight = heights.find(height => 
      height.name.toLowerCase().trim() === name.toLowerCase().trim() && 
    ...
    if (existingHeight) {
      return { isValid: false, error: "Height name already exists" };
    }
  ...
  const fetchHeights = async () => {
    try {
      setLoading(true);
      const response = await api.get('/heights?includeDeleted=true');
          ...
          <div className="makeFlex row gap10">
            <div className="fullWidth">
              <FormField
                ...
                placeholder="Enter Height Name (e.g., 50cm)"
                required={true}
              />
            </div>
          </div>
                      ...
                      return (
                        <>
                          ...
                          <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                            ...
                            <span className="detailValue font14 blackText appendLeft6">{height.name}</span>
                          </div>
                          {height.description && (
                            <div className="brandDetail makeFlex spaceBetween alignCenter paddingTop8 paddingBottom8">
                    ...
                    <tbody>
                      {currentHeights.map((height) => (
                        <tr key={height._id || height.id} className="tableRow">
                          ...
                          <td className="tableCell">
                            <span className="addressText" title={height.description}>
                              {height.description ? (height.description.length > 30 ? `${height.description.substring(0, 30)}...` : height.description) : '-'}
                            </span>
                          </td>
                          <td className="tableCell">
