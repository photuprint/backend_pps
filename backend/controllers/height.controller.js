...
export const createHeight = async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      res.status(400).json({ msg: 'Height name already exists' });
    } else {
      res.status(500).json({ msg: 'Failed to create height' });
    }
  }
};
...
export const updateHeight = async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      ...
      res.status(500).json({ msg: 'Failed to update height' });
    }
  }
};
