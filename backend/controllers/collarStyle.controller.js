...
const generateNextCollarStyleId = async () => {
  try {
    const lastCollarStyle = await CollarStyle.findOne({}, {}, { sort: { 'collarStyleId': -1 } });
...
export const getCollarStyleById = async (req, res) => {
  try {
    const collarStyle = await CollarStyle.findById(req.params.id);
    if (!collarStyle) {
...
export const createCollarStyle = async (req, res) => {
  try {
    ...
    // Check if collar style with same name already exists (excluding deleted ones)
    ...
    // Generate next collar style ID
    const collarStyleId = await generateNextCollarStyleId();
...
export const updateCollarStyle = async (req, res) => {
  try {
    ...
    // Check for duplicate names (excluding current collar style and deleted ones)
...
export const hardDeleteCollarStyle = async (req, res) => {
  try {
    const collarStyle = await CollarStyle.findByIdAndDelete(req.params.id);
    if (!collarStyle) {
      return res.status(404).json({ msg: 'Collar style not found' });
    }
